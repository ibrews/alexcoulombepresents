...
import { BOOKING_TIMEZONE, HOLD_HOURS, bookingActionSignature, type BookingRow } from "./config";
...
export async function sendBookingOwnerRequest(booking: BookingRow): Promise<void> {
  ...
  const body = [
    ...
    "",
    `Confirm (holds it ${HOLD_HOURS}h and emails them a payment link):`,
    confirm,
    "",
    "Decline (frees the slot and lets them know):",
    decline,
    "",
    `They have NOT been charged. $${(booking.price_cents / 100).toFixed(0)} is collected only after you confirm.`,
  ].join("\n");
  const { error } = await resend.emails.send({
    from: FROM,
    to: OWNER_EMAIL,
    replyTo: booking.email,
    subject: `Booking request: ${booking.name} — ${when}`,
    text: body,
    html: brandedHtml(body),
  });
...
export async function sendBookingConfirmedEmail(booking: BookingRow): Promise<void> {
  ...
  const body = [
    ...
    `  ${when}`,
    "",
    `Pay here to lock it in — $${(booking.price_cents / 100).toFixed(0)}, one hour:`,
    payUrl,
    "",
