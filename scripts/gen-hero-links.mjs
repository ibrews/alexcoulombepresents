...
function generatedEntries(source) {
  const match = source.match(/export const generatedHeroLinks: HeroLink\[\] = ([\s\S]+);\n?$/);
...
function reportStaleOutput(current) {
  const expectedEntries = generatedEntries(output);
  const currentEntries = generatedEntries(current);

  console.error(`\n${OUT} is stale; run \`npm run build && npm run gen:hero-links\`.`);
  if (!expectedEntries || !currentEntries) {
    console.error("Could not compare individual links because the generated file has an unexpected format.");
    return;
  }

  const expectedByHref = new Map(expectedEntries.map((entry) => [entry.href, entry]));
  const currentByHref = new Map(currentEntries.map((entry) => [entry.href, entry]));
  const added = [...expectedByHref.keys()].filter((href) => !currentByHref.has(href));
  const removed = [...currentByHref.keys()].filter((href) => !expectedByHref.has(href));
  const changed = [...expectedByHref.keys()].filter(
    (href) => currentByHref.has(href) && JSON.stringify(expectedByHref.get(href)) !== JSON.stringify(currentByHref.get(href))
  );
  const differences = [
    ...added.map((href) => `+ ${href}`),
    ...removed.map((href) => `- ${href}`),
    ...changed.map((href) => `~ ${href}`),
  ];

  console.error(`hrefs added         ${added.length}`);
  console.error(`hrefs removed       ${removed.length}`);
  console.error(`entries changed     ${changed.length}`);
  if (differences.length) {
    console.error(`first changed hrefs (of ${differences.length})`);
    for (const href of differences.slice(0, 20)) console.error(`  ${href}`);
  }
}
...
console.log(`\n${check ? "checked" : "wrote"} ${OUT}: ${generated.length} links ${JSON.stringify(genTiers)}`);
