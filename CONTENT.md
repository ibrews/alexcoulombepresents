...
- `[training.6]` ▸ Company & studio training · bundled curricula
- `[training.7]` ▸ Represent a team? This is the fast lane.
- `[training.8]` ▸ Custom multi-week curricula assembled from the 50+ ready-to-teach classes below — the same live, hands-on training delivered on behalf of Epic Games to their key partner studios, broadcast graphics teams, and enterprises. Live over Zoom or on-site, cloud workstations available for every learner, Q&A sessions woven between modules. Pricing varies with team size, class count, and scheduling — tell us what your team needs and you'll have a scoped quote fast.
- `[training.9]` ▸ or email info@alexcoulombepresents.com
- `[training.10]` ▸ FORMAT
- `[training.11]` ▸ Live, two-hour classes — à la carte or as curriculum bundles. Solo VIP deep-dives or team training for entire studios.
- `[training.12]` ▸ SCHEDULE
- `[training.13]` ▸ Intro classes typically start on the first Monday of the month at 1pm ET, with additional sessions throughout the week.
- `[training.14]` ▸ CREDENTIALS
- `[training.15]` ▸ Unreal Authorized Instructor + Authorized Training Center — certified by Epic Games.
- `[training.16]` ▸ Certified by Epic Games
- `[training.17]` ▸ Authorized, and re-authorized every year.
- `[training.18]` ▸ The curriculum
- `[training.19]` ▸ Twelve tracks. Every one books as a live two-hour session — intro tracks and specialized deep-dives, priced separately. Take one, take them all, or bundle them for your team above. Pricing shows at checkout in the store . Not sure what's next? Vote on the September cohort →
- `[training.20]` ▸ New
- `[training.21]` ▸ The full teaching catalog
- `[training.22]` ▸ 50+ classes, ready to run.
- `[training.23]` ▸ Every class below has been taught live — most many times over — as two-hour, instructor-led sessions on behalf of Epic Games to their key partner studios, broadcast teams, and enterprises. Book any of them as an open-enrollment session, or point at a column and say "that one, for my whole team."
- `[training.24]` ▸ Book a session in the store →
- `[training.25]` ▸ Bundle these for your team →
- `[training.26]` ▸ Try the teaching first
- `[training.27]` ▸ All
- `[training.28]` ▸ talks on YouTube →
- `[training.29]` ▸ From the Epic Games archives
- `[training.30]` ▸ Courses built for Epic Games.
- `[training.31]` ▸ Separate from Agile Lens, over eight years Alex collaborated on 50+ official Epic Games courses. Some of the once-gated ones are finally coming online to the public — here are a few worth your time.
- `[training.32]` ▸ AI training · its own track
- `[training.33]` ▸ Learn AI workflows, Unreal optional.
- `[training.34]` ▸ This is its own thing — you don't need to care about Unreal Engine to take it. Agile Lens runs a fleet of machines with AI agents shipping real production work around the clock without breaking the bank, and the plan is to teach that the way the Unreal classes already do: live, hands-on, on your own project, whatever you're building.
- `[training.35]` ▸ Learning Unreal with Alex? AI comes woven in. Here purely for AI? Unreal only shows up if your use case actually calls for it. Formal AI classes aren't scheduled yet — tell us what you'd want and you'll be first to know.
- `[training.36]` ▸ Watch the full talk →
- `[training.37]` ▸ Learn AI
- `[training.38]` ▸ Get notified the moment AI sessions open — and help shape what they cover.
- `[training.39]` ▸ Learn Unreal
- `[training.40]` ▸ Classes run now — but if you'd rather be pinged about upcoming cohorts than book today, hop on the Unreal list.
- `[training.41]` ▸ Why learn here and not from an on-demand video course?
- `[training.42]` ▸ It's alive. Classes adapt to your actual project. Bring your broken Blueprint, your janky lighting, your VR comfort problem — we fix real things in real time.
- `[training.43]` ▸ It's current. The new AI for Unreal Engine class covers MCP servers and AI build agents — practical workflows that we stay on the cutting edge of.
- `[training.44]` ▸ It's proven. The same techniques drove nine-figure real-estate sales, RSC rehearsals, and TestFlight-shipping Vision Pro apps. You're learning the production path, not the tutorial path.
- `[training.45]` ▸ Book a class — instant checkout →
- `[training.46]` ▸ Team / studio training
- `[training.47]` ▸ Unreal NYC
- `[training.48]` ▸ Alex and Agile Lens lead the NYC Unreal Engine meetup — talks, demos, pizza, and swag. Come meet the community before committing to a class.
- `[training.49]` ▸ agilelens.com/unrealnyc ↗
- `[training.50]` ▸ Epic community page ↗
- `[training.51]` ▸ Free stuff first
- `[training.52]` ▸ The YouTube channel ("I Wish I Learned This Sooner") and The (Unofficial) Unreal Engine Podcast are free. Start there, then bring your questions to class.
- `[training.53]` ▸ Podcast on all platforms ↗
- `[training.54]` ▸ Request a team quote →
- `[training.55]` ▸ Alex will follow up with a scoped quote — usually within a day.
...
### roles  (8 items — add or remove lines freely)
...
### timeline  (9 entries — add or remove whole entries)
...
- `[data.timeline.4.title]` ▸ Live performance goes immersive
- `[data.timeline.4.detail]` ▸ Ghosted — an award-winning volumetric AR piece on Magic Leap. 'Loveseat' at the Venice Biennale: the first live VR show viewable globally.
- `[data.timeline.5.title]` ▸ A Christmas Carol VR
- `[data.timeline.5.detail]` ▸ Two live actors, 45 minutes, avatar switching, facial capture — five holiday seasons on Quest, capped by a Raindance Immersive 2025 Official Selection, then retired as an annual show. The recorded replay is now finding its way to Apple Vision Pro.
- `[data.timeline.6.title]` ▸ Royal Shakespeare Company R&D
- `[data.timeline.6.detail]` ▸ A mixed-reality rehearsal application with the RSC. Theatre and XR, finally in the same room.
- `[data.timeline.7.title]` ▸ SIGGRAPH
- `[data.timeline.7.detail]` ▸ 'Seeing Yourself on Stage: Multi-Avatar Performance and the Evolution of Self-Monitoring in VR' — published research from a decade of live VR performance.
...
### products  (12 entries — add or remove whole entries)
...
- `[data.products.forage.sections.4.heading]` ▸ More on the way
- `[data.products.forage.sections.4.body]` ▸ A Chrome extension is in development: owned and wishlisted badges that appear directly on fab.com as you browse, so you never accidentally buy something you already have. Multi-engine support is planned — groundwork is already in for Unity and Godot alongside Unreal. Megascans and Cosmos integration is on the roadmap.
- `[data.products.forage.sections.5.heading]` ▸ Not a marketplace, not a generator
- `[data.products.forage.sections.5.body]` ▸ Forage doesn't sell assets and doesn't synthesize them. It's a really good scout and installer for the packs you already paid for — which it turns out is the thing everyone actually needed.
...
- `[data.products.renderman-bridge.sections.4.heading]` ▸ Where it goes next
- `[data.products.renderman-bridge.sections.4.body]` ▸ A two-way live connection is the destination: keep authoring in Unreal and watch RenderMan's progressive render update as you drag actors and lights — the same architecture as the UnRealityKit Live Link previewer, and the reason the render host is out-of-process by design. Also queued: AOVs and Cryptomatte through the button, a multi-machine render farm, and the harder inverse — RenderMan looks coming back as Unreal materials.
...
- `[data.externalLinks.3.note]` ▸ The NYC Unreal Engine meetup, led by Alex — talks, demos, and the only room in Manhattan where 'nanite' is small talk.
- `[data.externalLinks.3.extra.label]` ▸ Official Epic community page
- `[data.externalLinks.4.label]` ▸ The (Unofficial) Unreal Engine Podcast
- `[data.externalLinks.4.note]` ▸ Co-hosted with Jacob Feldman. Unreal news, takes, and guests.
...
- `[data.externalLinks.7.note]` ▸ Tutorials ('I Wish I Learned This Sooner'), Unity→UE5 migration guides, MetaHuman deep dives.
...
- `[store.storeItems.unreal-foundations-cohort.saleWindow.closesAtISO]` ▸ 2026-08-05T14:00:00Z
- `[store.storeItems.unreal-foundations-cohort.saleWindow.closedNote]` ▸ This cohort has already started — join the list for the next one.
