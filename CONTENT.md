...
- `[about.1]` ▸ Trained as a theatre architect. Constructing the cognitive ladder to virtual experiences that real venues can't reach.
...
- `[about.17]` ▸ Kids have names that each only rhyme with one other word in the English language. Has written and performed songs for them with slant rhymes that stretch credulity.
...
- `[training.2]` ▸ Alex is a top-rated Epic Games Authorized Instructor running Manhattan's first Unreal Authorized Training Center, in association with Agile Lens. He has taught 300+ courses on behalf of Epic Games to their key partners. These curriculums come from nearly a decade of real client work in Unreal: photoreal archviz, live virtual production, multiplayer VR, and now AI-driven workflows. You'll learn methodologies you won't find anywhere else.
- `[training.3]` ▸ FORMAT
...
- `[training.15]` ▸ Why learn here and not from a 40-hour video course?
...
- `[training.17]` ▸ It's current. The new AI for Unreal Engine class covers MCP servers and AI build agents — material that didn't exist a year ago and that Alex is actively building in public.
...
- `[training.22]` ▸ Alex leads the NYC Unreal Engine meetup — talks, demos, and the only room in Manhattan where "nanite" is small talk. Come say hi before committing to a class.
...
- `[repos.3]` ▸ More on github.com/ibrews — including the forks, the experiments, and the repos that should probably be archived.
...
- `[skills.1]` ▸ Teach your AI what took me a decade to learn.
...
- `[data.repos.MetaHumanGodot.tagline]` ▸ Interactive look-dev for MetaHuman characters in stock Godot 4.6 — live skin/lighting/hair sliders.
- `[data.repos.MetaHumanGodot.story]` ▸ MetaHumans rendered in stock Godot 4.6 Forward+ — no engine fork, no custom build. A real-time look-development and turntable tool: dial in skin, lighting, hair, and eyes with live sliders, and drive all 52 ARKit facial blendshapes. Bring your own MetaHuman (assets aren't redistributed — Epic EULA respected).
...
### products  (4 entries — add or remove whole entries)

- `[data.products.forage.name]` ▸ Forage
- `[data.products.forage.tagline]` ▸ Fab's ultimate companion. An AI-first asset scout for the Unreal Engine packs you already own.
...
- `[data.products.forage.sections.0.body]` ▸ Forage returns structured JSON designed to hand straight to an AI build agent — an in-editor MCP plugin, Cursor driving UE, or a human reading the output. 'You already own Medieval Castle Walls Pro; hero asset is SM_CastleGate; the UE 5.5 artifact is the one you want.' Your agent builds; Forage scouts.
- `[data.products.forage.sections.1.heading]` ▸ A wishlist without the cap
- `[data.products.forage.sections.1.body]` ▸ Fab caps your wishlist around 150 items. Forage keeps an unlimited local one — and tells you when a wishlisted pack lands in your owned library. For things you don't own yet, it opens Fab in your browser so you can shop like a human.
- `[data.products.forage.sections.2.heading]` ▸ Not a marketplace, not a generator
- `[data.products.forage.sections.2.body]` ▸ Forage doesn't sell assets and doesn't synthesize them. It's a really good scout for the packs you already paid for — which it turns out is the thing everyone actually needed.
...
- `[data.products.unrealitykit-bridge.sections.0.heading]` ▸ What's already real
- `[data.products.unrealitykit-bridge.sections.0.body]` ▸ The live arc is hardware-verified end-to-end on Apple Vision Pro: UE boots headless inside the visionOS shell, a full room mirrors across the bridge with translated materials, lights mirror over the seam, audio is positional, and input flows back — tap-poke, pinch-grab-and-throw into Chaos physics, rotate-with-hand. One live simulation presents through a Mixed / Immersive / Volume (tabletop miniature) picker. At 90 fps. On device.
- `[data.products.unrealitykit-bridge.sections.1.heading]` ▸ The material translator
- `[data.products.unrealitykit-bridge.sections.1.body]` ▸ The hardest single problem: every UE material has to become a RealityKit ShaderGraph. The bridge translates UE material graphs to MaterialX — texture binding, colorspace, lighting response, normal-map conventions, alpha and opacity, dynamic parameters — each milestone render-verified against ground truth.
- `[data.products.unrealitykit-bridge.sections.2.heading]` ▸ Why it matters
- `[data.products.unrealitykit-bridge.sections.2.body]` ▸ For polished mixed reality on Vision Pro, rendering with RealityKit beats fighting the compositor. A flagship Disney/Marvel production validated the architecture; an open bridge makes it available to everyone else. (The repo is private per Epic's Unreal Engine source-access policy — Epic-linked collaborators can request access.)
...
- `[data.agentSkills.2.name]` ▸ godot-visionos
- `[data.agentSkills.2.blurb]` ▸ Build Godot apps for Apple Vision Pro — flat-plane and full immersive.
- `[data.agentSkills.2.detail]` ▸ Everything learned shipping Cascade Countdown: the build switcher, simulator input, hand tracking, and the silent-failure traps that cost device round-trips.
...
- `[data.agentSkills.5.name]` ▸ metahuman-godot-pipeline
- `[data.agentSkills.5.blurb]` ▸ The complete MetaHuman → Blender → Godot conversion workflow, as a skill.
- `[data.agentSkills.5.detail]` ▸ The paid counterpart to the free MetaHumanGodot viewer: full export, ARKit morph-target bake, material reassembly, and the shader stack — with real scripts, not prose. In final review.
- `[data.agentSkills.6.name]` ▸ ue5-mcp
- `[data.agentSkills.6.blurb]` ▸ A field manual for AI agents driving Unreal Engine 5 through MCP.
- `[data.agentSkills.6.detail]` ▸ Free and open source. Reflection gotchas, crash patterns, subsystem quirks — install it and your agent stops rediscovering UE's sharp edges every session.
- `[data.agentSkills.6.link.label]` ▸ GitHub
- `[data.agentSkills.7.name]` ▸ apple-platform-skills
- `[data.agentSkills.7.blurb]` ▸ Claude Code skills for visionOS SharePlay, SpriteKit, and GameKit multiplayer.
- `[data.agentSkills.7.detail]` ▸ Free and open source. npx skills add ibrews/apple-platform-skills — and your agent knows the session.join() trap before it falls in.
- `[data.agentSkills.7.link.label]` ▸ GitHub

### videos  (16 entries — add or remove whole entries)
