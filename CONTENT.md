...
- `[home.12]` ▸ 01 / ENGINES
- `[home.13]` ▸ Unreal & Godot & even ThreeJS, let's go further than the documentation
- `[home.14]` ▸ Engine-level improvements for Vision Pro, Godot looking gorgeous at 90 fps with custom source code, WebXR and WebGPU doing the impossible. MetaHumans up close & virtual like you've never seen them before. When a plugin says "Experimental," the real fun begins.
- `[home.15]` ▸ 02 / AI
- `[home.16]` ▸ Agents that ship real work. No tech for tech's sake.
- `[home.17]` ▸ A fleet of machines at Agile Lens running AI agents around the clock without breaking the bank. Field manuals so agents drive Unreal without crashing it. Pipelines where the overnight build greets you with a new TestFlight-approved app waiting on device by morning.
- `[home.18]` ▸ 03 / LIVE VR
- `[home.19]` ▸ Theatre is the original VR
- `[home.20]` ▸ Live actors performing in headsets since 2018. Worked on Venice Biennale's first global live VR show, an annual Christmas Carol in VR, mixed reality theatre with an actor surrounded by digital assets, and published SIGGRAPH papers on all of it.
- `[home.21]` ▸ Training for companies & studios
- `[home.22]` ▸ Bring your whole team. We'll build the curriculum.
- `[home.23]` ▸ Custom curricula bundled from 50+ ready-to-teach classes — the same live training delivered to Epic Games' key partners, broadcast teams, and AAA studios. Remote or on-site, cloud workstations available, pricing scoped to your team.
- `[home.24]` ▸ Get a team quote →
- `[home.25]` ▸ Fresh from the public repos
- `[home.26]` ▸ Real tools built for Agile Lens , now available to you.
- `[home.27]` ▸ all repos →
- `[home.28]` ▸ The Lab
- `[home.29]` ▸ Here's what's bubbling:
- `[home.30]` ▸ all lab projects →
...
- `[repos.3]` ▸ More on github.com/ibrews — including the forks, the experiments, and the hyper-targeted repos to solve absurdly specific problems.
